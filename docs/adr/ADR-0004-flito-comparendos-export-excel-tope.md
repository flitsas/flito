...
**Método.** `sendExcel` con filas sintéticas de la forma real —todas las columnas de `COLUMNAS_EXPORT`, textos distintos por fila para que `exceljs` no comprima la tabla de cadenas— sobre un sumidero que drena, como hace el socket real. Se mide el delta de RSS sobre el reposo del proceso, el pico de heap y el retraso real de un `setInterval` de 20 ms (lag del event loop). Instrumento en `apps/api/__tests__/helpers/export-coste.ts`; escenarios en `flito-comparendos-export-coste.test.ts` (secuencial) y `flito-comparendos-export-concurrencia.test.ts` (simultáneo). **Lo que el repo reproduce solo es una parte de la tabla:** esos dos tests fijan un export y dos simultáneos **al tope vigente**; el resto de las celdas (los otros tamaños, y de tres a cinco simultáneos) salen de corridas puntuales del mismo instrumento durante la HU, y repetirlas exige cambiar a mano el tope y el número de lotes. **Cada escenario se corre en un proceso propio**: Vitest reutiliza los workers entre archivos y un proceso que ya construyó un workbook grande arranca el siguiente con las arenas del allocator calientes, lo que hace que el delta salga optimista — así se midió la primera vez, en la HU #11558, y por eso su «peor caso» daba un delta MENOR que su caso realista.
...
**Cómo se lee, y de qué cuelga.** El presupuesto no es 512 MB: es **512 − lo que el API ya ocupa**. Con el extremo pesimista del rango del PR #153 (250 MB en régimen) quedan **262 MB** para la generación. Ese 250 —`REGIMEN_API_MB` en `apps/api/__tests__/helpers/export-coste.ts`— es una **estimación heredada del PR #153, no una medida del proceso de hoy**, y todo lo que sigue cuelga de ella: si el régimen real fuera otro, el presupuesto de 262 MB y todas las fronteras de esta sección se mueven con él. Medir el RSS en régimen del API desplegado es la forma barata de convertir esta sección en dato. Con 5 000 filas, dos exports simultáneos consumen 247 de esos 262 — a **15 MB** del `max_memory_restart`— y tres lo cruzan (615 MB proyectados). Y esos dos simultáneos no son hipótesis **ni necesitan dos personas**: `exportLimiter` acota **peticiones por minuto, no peticiones en vuelo** (`windowMs: 60_000, max: 5` en `flito-comparendos.routes.ts`), con cuota **por usuario** (`keyGenerator: userOrIpKey(…)`) y sin ninguna cota global ni semáforo, así que una sola cuenta puede tener varios exports construyéndose a la vez en el mismo proceso.
...
**Decisión: `COMPARENDOS_EXPORT_MAX_FILAS` pasa de 5 000 a 2 000**, que es la **Opción B** de este mismo ADR — la que se descartó «por poco» y sobre la que quedó escrito que bajar a ella «es un cambio de una línea y no necesita otro ADR».

**Qué distingue esta tabla y qué no, dicho antes de que alguien la cite con más resolución de la que tiene.** Sostiene con solidez que **5 000 sobra**: 247 MB frente a 106 MB con dos simultáneos es un factor 2,3 que ningún ruido de medición explica. **No** sostiene 2 000 frente a 3 000: un export de 3 000 filas mide **+58 MB** y uno de 2 000, **+93 MB** —un archivo más pequeño costando más RSS—, y con dos simultáneos son 116 frente a 106. A esa escala manda el **ruido del allocator y del GC**, no el número de filas, así que las diferencias dentro de la banda 2 000–3 000 están dentro del ruido — la fila de 2 500 lo confirma: +91 MB aislada, por encima de los +58 MB de 3 000. 2 000 se elige por ser la Opción B ya analizada aquí, por ser el extremo conservador de esa banda y por dejar margen bajo el presupuesto; **no** porque la medición lo separe de 3 000. Las contras de la Opción B siguen siendo las de su tabla y no se maquillan: un NIT grande con varios años de histórico puede no caber en un archivo y el usuario trocea por filtro. El techo de extracción por minuto baja a la vez de 25 000 a 10 000 filas, que es una mejora de privacidad, no un daño colateral.
...
**Lo que esto NO resuelve, y es la parte que hay que leer.** Bajar el tope acota el coste por export; **no acota la concurrencia**, porque nada la acota. Con 2 000 filas, **cinco exports simultáneos todavía caben**: +239 MB de los 262, o sea 23 MB de margen — es el último dato medido de la tabla y el más ajustado de todos. El **sexto no está medido**: extrapolando la pendiente de la propia tabla (+45 MB del tercero al cuarto, +70 MB del cuarto al quinto) cruzaría el techo, pero eso es una proyección y no una medición, y el margen de los cinco cuelga además de que el régimen del API sean los 250 MB estimados. La frontera que se puede afirmar es esa y solo esa: **cinco entran con 23 MB de sobra, y a partir de ahí no hay dato**. Y llegar a los cinco no requiere cinco personas coordinadas: como `exportLimiter` cuenta peticiones **por minuto y no peticiones en vuelo**, basta **una sola sesión de administrador** —comprometida, guionizada, o alguien con prisa pulsando cinco veces— para poner los cinco exports a la vez en el heap del proceso. Ningún valor del tope arregla eso: es una propiedad del diseño —workbook entero en memoria, sin cota global— y no de la calibración. Las dos salidas que sí lo arreglarían (semáforo en proceso que serialice la generación, o `stream.xlsx.WorkbookWriter` escribiendo incremental sobre `res`) quedaron **fuera del alcance** de la HU #11651 por decisión del Líder Técnico el 2026-08-20. Este párrafo existe para que el día que aparezca un reinicio de PM2 correlacionado con la ruta, nadie tenga que volver a descubrir por qué.
