...
**Aprobado** — 2026-09-09. Aprobador: **David Chica**. Feature [#12072](https://dev.azure.com/FlitDevOps/FLIT%20-%20FLITO/_workitems/edit/12072) («Módulo de usuarios: roles configurables, permisos por función y borrado lógico»), HU [#12171](https://dev.azure.com/FlitDevOps/FLIT%20-%20FLITO/_workitems/edit/12171).

Con la aprobación quedan **desbloqueadas** las HU #12084, #12087, #12089 y #12171, que escriben contra el mecanismo que aquí se decide y que no podían empezar mientras este ADR estuviera en `Propuesto`.
...
- **Dónde se decide cada cosa** — tres asuntos distintos, tres dueños:
  ...
  3. **Mecanismo** de ejecución de la retención declarada (incluida la de `audit_logs`) → HU [#12215](https://dev.azure.com/FlitDevOps/FLIT%20-%20FLITO/_workitems/edit/12215), *«[BACKEND] – Retención documental – Aplicar la retención declarada en vez de simularla»*, creada el 9/09/2026 sin Feature padre y fuera del alcance del #12072. Citar su ID es requisito de cierre del AC5 de la #12171.
     **Corrección al título que este ADR proponía** («convertir el cron PESV de DRY-RUN a purga real»): medido sobre `develop` el 9/09/2026, de las **siete** políticas sembradas en `0060:225-233` solo **una** (`checklist`) es `purgar`; tres archivan fuera de línea (`incidente_vial`, `manifiesto`, `acta_comite` — y `audit_log`, cuatro con ella) y dos anonimizan (`alcohol_test`, `pii_access_log`). Un cron que purgara las siete destruiría evidencia que la ley obliga a conservar, así que la historia exige acción por acción. Segundo hallazgo de esa medición: la siembra de la `0060` es **condicional** (`WHERE EXISTS (SELECT 1 FROM users WHERE id = 1)`), de modo que su presencia en cada ambiente hay que comprobarla, no darla por hecha.
...
- **ADR-0005** (`Aprobado` el 2026-09-09, el mismo día que este) — este ADR es su **segunda aplicación**, y una de las razones por las que su regla general dejó de estar en `Propuesto`: clasifica las dos FKs nuevas como «auditoría / prueba» → `RESTRICT` explícito, y documenta un caso donde la regla **no** aplica porque la FK no apunta a `users` (`titular_rol_codigo`). No lo contradice ni lo modifica.
- **ADR-0015** (`Aprobado` el 2026-09-09, HU #12169) — apareció en el árbol de trabajo mientras se escribía este ADR. Convierte `users.role` en `varchar(40)` con FK hacia `permisos_roles(codigo)`. **Compatible sin cambios**: este ADR guarda `rol_afectado_codigo varchar(40)` y deliberadamente **sin** FK (§5), así que la conversión de ADR-0015 no lo afecta y este no interfiere con CF-05. Si ADR-0015 cambiara la PK a `serial` (su Opción 2, descartada), habría que revisar §5 de este.
