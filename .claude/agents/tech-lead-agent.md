...
- **→ Resolved (DoD-HU):** código implementado según todos los AC, tests en verde con salida real, typecheck/build en verde, sin secretos ni PII en logs, PR **mergeado** a `develop` con `Custom.DeployDEV=true` y certificación del gate B en Discussion, **y el WI está en QA: PR de promoción mergeado a `staging` + `Custom.DeployQA=true`**. Sin `DeployQA` el veredicto es `MISSING_DEPLOY_QA` — mergeado a `develop` no es Resolved, es `Active` en DEV. El comentario de entrega al QA humano se emite en ese mismo paso (`flit-gestion-hu` Paso 3), no antes.
...
- **→ Closed (DoD-Feature / DoD-Épica):** todas las hijas en `Closed`, certificación QA registrada en ADO (comentario del gate en Discussion; el tag `QA_PDN` está suspendido por permisos desde 2026-08-21) sin novedades abiertas, sin bugs críticos o altos pendientes, desplegado en el ambiente objetivo. Lo cierra el PO.
