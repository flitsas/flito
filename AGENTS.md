...
- **Merge a `develop`:** lo ejecuta el **`pr-monitor-agent`** (o el hilo si el subagente no pudo) vía MCP `github` (`merge_pull_request`, merge commit) cuando: base exactamente `develop`, CI `build + test` + `dependency-audit` + `secret-scan` + `naming` + `pr-gates` en verde, sin conflictos. Abrir el PR a `develop` durante el desarrollo **es** la autorización; no se espera un segundo «sí». Opt-out: el humano dijo «no mergees». `flit-code-review` y `qa-agent` B son **pre-PR** (no se abre el PR sin `OK`/`PASS`); el monitor **no** espera un HANDOFF de QA porque ese gate ya cerró. Tras merge → `flit-integration-ado` Modo B (Deploy DEV). **El work item sigue en `Active`**: `Resolved` llega cuando el desarrollo entra al ambiente de QA (`staging`), no con el merge a `develop` — ver «Jerarquía y cascada de estados». En **cadena apilada**, si el CI de merges intermedios queda `cancelled` por concurrency, el gate de Deploy es el tip de `develop` que ya incluye la cadena (detalle en la skill).
- **Merge a `staging` / `release`:** siempre humano (`flit-release`). Ningún agente mergea promociones. **Hard-stop:** no se crea el PR `develop → staging` si el Feature no tiene evidencias DEV `COMPLETAS` (`flit-evidencias-dev`: sesión conjunta + capturas en el Feature). Tras el merge a `staging` → Modo B (`DeployQA`) → **`flit-gestion-hu` Paso 3 (tag `QA` + aviso a Daniel Amado + `Resolved`) por cada HU/Bug promovido** → cascada a Feature y Épica (Paso 4 de la skill).
- **El PR abierto no detiene la cadena:** tras `create_pull_request` + Modo A + `pr-monitor-agent` en background, la **siguiente HU arranca de inmediato** (rama desde la rama previa). Si un check del PR anterior se pone rojo (`CI-ROJO` con causa `CODIGO`), la HU en curso **se pausa momentáneamente** (WIP commiteado en su rama, sin push obligatorio), se corrige el eslabón anterior en **su** rama, se relanza el monitor, y luego se rebasea la HU en curso sobre la corrección y se continúa. Protocolo detallado: `flit-modo-desarrollo-auto` «Pausa por CI rojo».
- Cerrar un Feature (`Closed`) es exclusivo del Product Owner. `Resolved` de Feature y de Épica lo pone el agente por cascada (con «sí» humano para escribir en ADO), nunca `Closed`.
...
En CI (`.github/workflows/ci.yml`) existen cinco checks de gate: **`build + test`**, **`dependency-audit`**, **`secret-scan`**, **`naming`** (rama, título del PR y trazabilidad HU/Bug; solo corre en PRs) y **`pr-gates`** (veredictos pre-PR declarados en el cuerpo del PR: `code-review`/`qa`/`security`/`db-review`; solo PRs HU/BUG) — los cinco deben estar en verde para mergear (precondiciones de `flit-integration-ado`). La suite completa de tests en CI **no** se sustituye por el mínimo filtrado local.
...
**Auto-reporte obligatorio:** el HANDOFF de cada agente declara su cifra (`Reconocimiento: N`) y el
hilo la contrasta con el presupuesto y la registra en el ledger del WI. Un presupuesto que nadie
reporta no existe — la mediana real triplicaba el presupuesto (26-ago a 8-sep) precisamente porque
nadie la declaraba.
...
- Ciclo de un work item de desarrollo (**HU o Bug** — ver «Paridad HU ↔ Bug»): `flit-gestion-hu` (Active → implementación + gates pre-PR incluido `qa-agent` B → PR → merge a `develop` = DEV, sigue `Active` → promoción a `staging` = QA → `Resolved`). Creación: `flit-crear-hu` (HU y Bug). Registro PR ↔ ADO y Deploy DEV/QA/PDN: `flit-integration-ado`. Ciclo completo por Feature: `flit-modo-desarrollo-auto` (**cadena apilada por defecto**; merge a `develop` al verde — ver Git flow). **Anti-estancamiento:** tras abrir el PR, delegar el monitoreo de CI y el merge al verde en `pr-monitor-agent`; **no** lanzar `qa-agent` B en paralelo al monitor (ese gate ya es pre-PR); en paralelo arrancar la siguiente HU — **prohibido** quedarse idle pidiendo «continúa» solo porque el CI está en curso.
- Features/HUs en ADO: audiencia PO + Tech Lead + desarrollo. **Funcional primero** (objetivo, flujo, criterios); **técnico al final** (módulos, esquema). Dueño de la redacción: `tech-lead-agent` (A/B) + `flit-crear-hu`. **Las Épicas no las escribe ningún agente**: las redacta el PO; los agentes las **leen** y de ellas derivan los Features (hijos de la Épica) y las HUs (hijas del Feature).

### Jerarquía Épica → Feature → HU/Bug y cascada de estados

El board tiene tres niveles y el estado sube y baja por ellos. **El QA humano prueba en el ambiente de QA (`staging`)**, nunca en DEV: `Resolved` significa «está en QA, listo para que el QA lo valide», y por eso no se pone al mergear a `develop`.

| Nivel | Quién lo crea | Pasa a `Active` | Pasa a `Resolved` | Vuelve a `Active` | `Closed` |
|---|---|---|---|---|---|
| **Épica** | PO (los agentes solo leen) | Al activar su primer Feature (`flit-gestion-hu` Paso 1, «padre primero») | Cuando **todos** sus Features están `Resolved`/`Closed` (Paso 4) | Si un Feature suyo vuelve a `Active` | PO |
| **Feature** | `tech-lead-agent` A **desde la Épica** (hijo de ella) | Al activar su primera HU/Bug (Paso 1) | Cuando **todas** sus HUs/Bugs hijos están `Resolved`/`Closed` (Paso 4) | Si el QA reactiva una HU hija o radica un Bug bajo el Feature/HU (Paso 5) | PO |
| **HU / Bug** | `tech-lead-agent` B + `flit-crear-hu` (HU); `qa-agent` C con pedido explícito (Bug) | Al empezar a implementar (Paso 1) | Cuando **llega a QA**: PR de promoción mergeado a `staging` y `DeployQA=true` (Paso 3) | El QA lo reactiva por hallazgo, o se corrige un Bug hijo | PO/QA |

Reglas de la cascada (todas en `flit-gestion-hu`, con «sí» humano para cada escritura en ADO):

1. **Activar hacia arriba:** activar una HU o Bug exige Feature padre `Active`, y activar un Feature exige Épica padre `Active` (si existe). Se sube lo que falte, con comentario de inicio en cada nivel; lo que ya está `Active` o superior no se rehace. Un Bug sin padre se activa igual y se declara la ausencia.
2. **Resolver hacia arriba:** al pasar una HU/Bug a `Resolved` se consulta a sus hermanos (WIQL de hijos del Feature); si **todos** quedan `Resolved`/`Closed`, el Feature pasa a `Resolved` con comentario; y lo mismo, un nivel más arriba, para la Épica. Un Feature con una HU `New`/`Active` **no** se resuelve aunque las demás lo estén.
3. **Reactivar hacia arriba:** cuando el QA humano (en `staging`) reactiva una HU o radica un Bug asociado al Feature/HU, el Feature vuelve a `Active` (y la HU, si el Bug cuelga de ella); si la Épica estaba `Resolved`, vuelve a `Active`. Cuando el Bug se corrige, se mergea a `develop` y **se promueve a `staging`**, el Bug pasa a `Resolved` y la cascada del punto 2 devuelve el Feature (y la Épica) a `Resolved`.
4. **Merge a `develop` ≠ Resolved.** Con el PR en `develop` el WI queda `Active` + `DeployDEV=true` + `Custom.Commits` «Integrado». El aviso al QA humano (mención `mailto:`) va **solo** en el Paso 3, cuando el WI ya está en `staging`. Mencionar al QA con el código en DEV es el error que esta sección corrige.
5. **Bug huérfano (definición vigente):** Bug cuya corrección **ya está en `staging`** y sigue en `Active`, o Bug `Resolved` cuyo Feature padre sigue `Active` con todos los hijos resueltos. Ambos se cierran con la skill, no se consultan como si no hubiera proceso.
...
- **Activar una HU exige que su Feature padre esté `Active`, y el Feature exige su Épica `Active`**: la skill que activa la HU (`flit-gestion-hu` / `flit-modo-desarrollo-auto`) sube la cadena de padres de `New` a `Active` con comentario de inicio en cada Discussion (si ya está `Active`, no rehacer; si la HU no tiene padre, se declara en el comentario). Un **Bug** con padre sigue la misma regla; un Bug **sin** padre se activa igual y se declara la ausencia. Lo valida `tech-lead-agent` modo C en el DoR. El cierre (`Closed`) del Feature y de la Épica sigue siendo exclusivo del Product Owner; el `Resolved` de ambos lo pone la cascada de `flit-gestion-hu` Paso 4.
...
`New → Active` (con comentario de inicio; padres `Active` primero) `→` implementación por
`backend-agent`/`frontend-agent` `→` `qa-agent` gate B + `flit-code-review` (pre-PR; FAIL/BLOQUEADO
= retrabajo **antes** del PR) `→` PR `→` `flit-integration-ado` Modo A `→` merge a `develop`
(`pr-monitor-agent`) `→` Modo B (`DeployDEV`; el WI **sigue `Active`**) `→` `devops-agent` M1
`→` **`flit-evidencias-dev`** (sesión conjunta en DEV; capturas en el Feature; sin `COMPLETO`
no hay promoción) `→` promoción humana a `staging` (`flit-release`) `→` Modo B (`DeployQA`)
`→` **`Resolved`** (tag `QA` + comentario a **Daniel Amado**, que prueba en `staging`; el
agente QA B ya corrió) `→` cascada Feature/Épica. **`Closed` lo pone el PO/QA**, nunca un
agente.

**Hard-stop — Bug huérfano:** ningún Bug queda en `Active` después de que su corrección **llega a
`staging`**, ni pasa a `Resolved` sin el comentario de cierre y sin el gate `qa-agent`. «Ninguna
skill mueve el State de un Bug» dejó de ser cierto: lo mueve `flit-gestion-hu`, igual que en una
HU. Un Bug promovido y sin `Resolved` es fallo de proceso, no una zona gris. Un Bug **mergeado a
`develop` y aún en `Active`** es, en cambio, el estado correcto hasta la promoción.
...
| Evidencias de flujo en DEV (capturas en el Feature) | skill `flit-evidencias-dev` |
| Promoción develop→staging→release | skill `flit-release` |
| ADO (conexión) | skill `flit-azure-devops` |
| ADO (crear HU o Bug / ciclo del work item / PR ↔ ADO / evidencias DEV / feature completo) | skills `flit-crear-hu` / `flit-gestion-hu` / `flit-integration-ado` / `flit-evidencias-dev` / `flit-modo-desarrollo-auto` |
...
| Épica en el board sin Features / descomponer HUs / DoR | Planear o refinar backlog **a partir de la Épica** (se lee, no se redacta): Features hijos de la Épica, HUs hijas del Feature. **P9:** ronda de cierre contra código/spec **antes** de crear HUs; corte = ítems del pedido, no capas | `tech-lead-agent` | HUs mal cortadas / alcance descubierto en desarrollo / Features huérfanos sin Épica |
...
| Ciclo ADO Active→Resolved | Activar al empezar (padres `Active` primero: Feature y Épica); **`Resolved` cuando llega a `staging`** (tras el merge de promoción + `DeployQA`), no al mergear a `develop`; **cascada** Feature/Épica al resolver y al reactivar. Plantillas | **Skill** `flit-gestion-hu` | Estados huérfanos / **Bugs huérfanos** / QA avisado con el código en DEV / Feature con todos los hijos resueltos y aún `Active` |
...
| Hallazgo formal / novedad (ambiente QA = `staging`, u otra etapa post-entrega) | Radicar Bug **nuevo** solo con **pedido explícito del QA**; el Bug cuelga del Feature/HU y **reactiva** a sus padres (`flit-gestion-hu` Paso 5) | `qa-agent` **modo C** + **Skill** `flit-gestion-hu` | Bug inventado en el ciclo de desarrollo / Feature que sigue `Resolved` con un Bug abierto |
...
| Tras M1 en DEV / antes de promover a QA | Feature (o HU suelta) en DEV; hay que recorrer el flujo **con el humano** y dejar capturas en el Feature | **Skill** `flit-evidencias-dev` | Promoción a QA sin evidencias / capturas solo en la HU / Vitest presentado como evidencia de flujo |
| Promoción staging/release | Pedido de promover **y** Feature con evidencias DEV `COMPLETAS` | `flit-release` (+ qa D + devops post-merge). Sin `flit-evidencias-dev` = **NO-GO**. Tras el merge humano a `staging`: Modo B `DeployQA` + **`flit-gestion-hu` Paso 3 (tag `QA` + @Daniel Amado + `Resolved`)** + cascada Feature/Épica | Promoción sin evidencias DEV / WIs en `staging` que siguen `Active` / QA sin tag ni aviso |
...
| Dejar el Bug en `Active` tras promover su corrección a `staging`, o preguntar «¿lo paso a Resolved?» como si no hubiera proceso | Skill `flit-gestion-hu` cierra el Bug igual que una HU (Paridad HU ↔ Bug) | **Alta (Bug huérfano)** |
| Promover a QA sin sesión `flit-evidencias-dev` / sin capturas en el Feature | Skill `flit-evidencias-dev` (COMPLETO) **antes** de `flit-release` | **Alta** |
| Pasar una HU/Bug a `Resolved` al mergear a `develop`, o mencionar al QA humano con el código solo en DEV | `Resolved` + tag `QA` + aviso a **Daniel Amado** **cuando está en `staging`** (`flit-gestion-hu` Paso 3 tras `flit-release`); en DEV queda `Active` + `DeployDEV` + evidencias en el Feature | **Alta** |
| Resolver una HU y dejar el Feature `Active` con todos los hijos resueltos (o la Épica con todos los Features resueltos) | Cascada `flit-gestion-hu` Paso 4 | Alta |
| Radicar un Bug de QA (modo C) sin devolver el Feature/HU a `Active` | `flit-gestion-hu` Paso 5 (reactivación hacia arriba) | Alta |
| Redactar o editar una Épica, o crear un Feature sin colgarlo de su Épica | La Épica la escribe el PO; `tech-lead-agent` A la lee y crea el Feature como hijo | Alta |
| Quedarse esperando el CI del PR sin arrancar la siguiente HU, o seguir apilando encima de un `CI-ROJO` de código sin corregir el eslabón anterior | Pista B de inmediato; ante `CI-ROJO` → «Pausa por CI rojo» de `flit-modo-desarrollo-auto` | Alta |
...
**Ledger por HU o Bug (recomendado en modo auto):** al cerrar cada eslabón, listar en el reporte del hilo: `gestion-Active ✅|❌ · impl=Agent✅|❌ · qa=HANDOFF✅|SIN-ENTORNO|FAIL-retrabajo|❌ · code-review=Skill✅|❌ · security/db ✅|N/A · integration-A=Skill✅|❌ · pr-monitor=Agent MERGED|CI-EN-CURSO|CI-ROJO|CONFLICTO|❌ · integration-B=Skill✅(DeployDEV)|N/A · M1=Agent✅|N/A · evidencias-dev=Skill COMPLETO|PARCIAL|BLOQUEADO|N/A · estado=Active-en-DEV✅|Resolved✅(staging)|❌`. `pr-monitor=MERGED` es el único éxito de ese eslabón (`LISTO-PARA-MERGE` no cuenta). `estado=Active-en-DEV✅` es el cierre normal de la ráfaga de desarrollo; `Resolved✅(staging)` solo lo marca la promoción (`flit-release` → `flit-gestion-hu` Paso 3). Sin `evidencias-dev=COMPLETO` en el Feature → **no** `flit-release`. `code-review=✅` solo con veredicto **OK**; `security=✅` solo con **PASS**; `db=✅` solo con **SANO**; `qa=✅` solo con **PASS** **y antes del PR**. `*-CON-OBSERVACIONES` **no** es ✅. Sin `qa=✅`/`SIN-ENTORNO` → **no** abrir el PR. `FAIL-retrabajo` = gate rojo sin Bug; corregir antes del PR.

**Operación solo-merge** («mergea los PRs», Modo B en lote): no inventar arquitectura/código; sí completar `flit-integration-ado` Modo B y **después** `devops-agent` M1 sobre el tip. Si las HUs o Bugs mergeados no tienen evidencia de `qa-agent`, declararlo en el reporte final («QA pendiente en: …») — no fingir que se ejecutó. Tras un merge a `develop` los WIs quedan `Active` (correcto); tras un merge a `staging`, los que sigan `Active` son «sin cerrar: …» y se resuelven con `flit-gestion-hu` Paso 3.

Los subagentes no pueden invocar a otros subagentes: cada uno devuelve un bloque `HANDOFF` y el hilo principal continúa. Gates humanos que **nunca** se omiten: activar una HU o Bug, crear rama/commit/push, abrir PR, **sesión `flit-evidencias-dev` (probar el flujo en DEV con el humano)**, merge a `staging`/`release`, cerrar un Feature, instalar herramientas, desplegar. El merge a `develop` en desarrollo lo ejecuta el **`pr-monitor-agent`** cuando el CI está verde, **sin** un segundo «sí». Opt-out: el humano dijo «no mergees». **No es gate** pedir al humano que despierte el hilo tras abrir el PR o mientras el CI está `pending` — ver Anti-estancamiento en `flit-modo-desarrollo-auto`.
