...
- **Merge a `staging` / `release`:** siempre humano (`flit-release`). Ningún agente mergea promociones. Tras el merge a `staging` → Modo B (`DeployQA`) → **`flit-gestion-hu` Paso 3 (`Resolved` + aviso al QA humano) por cada HU/Bug promovido** → cascada a Feature y Épica (Paso 4 de la skill).
...
`→` promoción humana a `staging` (`flit-release`) `→` Modo B (`DeployQA`) `→` **`Resolved`**
(comentario de entrega al QA **humano**, que prueba en `staging`; el agente QA ya corrió) `→`
cascada Feature/Épica. **`Closed` lo pone el PO/QA**, nunca un agente.
...
| Promoción staging/release | Pedido de promover | `flit-release` (+ qa D + devops post-merge). Tras el merge humano a `staging`: Modo B `DeployQA` + **`flit-gestion-hu` Paso 3 por cada WI promovido** + cascada Feature/Épica | Promoción sin gates / WIs en `staging` que siguen `Active` / QA sin aviso |
...
| Pasar una HU/Bug a `Resolved` al mergear a `develop`, o mencionar al QA humano con el código solo en DEV | `Resolved` + aviso al QA **cuando está en `staging`** (`flit-gestion-hu` Paso 3 tras `flit-release`); en DEV queda `Active` + `DeployDEV` | **Alta** |
