...
6. En una fila, pulse **Ver**. En adquisición, **Cargar factura** (un archivo) o, desde el encabezado, **Cargar facturas (masivo)**.
...
8. Como Administrador, en una solicitud en **Pendiente de revisión**: en el detalle verá una ficha de solo lectura con lo que contestó (o no) el RUNT — **Esperando al RUNT**, **RUNT no disponible**, **Sin registro**, **No cuadra**, **Coincide con el RUNT** o **SOAT vigente**. Mientras espera, marca, línea, modelo y organismo salen como **—**: no es un error. Si hay **SOAT vigente**, lo habitual es rechazar; **Validar** sigue disponible si es una excepción. Pulse **Validar** y elija a quién se envía (**Gestionado por Operaciones** o un proveedor) — la solicitud pasa a **Solicitado**. O pulse **Rechazar** y elija una **causal** de la lista y escriba una **observación**: las dos son obligatorias, y el Cliente las lee tal cual. Si la ficha sigue en espera, **Actualizar verificación** vuelve a cargar el detalle; no relanza una consulta nueva.
9. Como Cliente, si su compañía tiene el canal abierto, pulse **Solicitar SOAT** en el encabezado: se abre el formulario. Escriba la placa, el VIN, el **tipo y número de documento** y el nombre del propietario, el contacto si lo tiene, y adjunte la factura de venta en PDF. Pulse **Enviar la solicitud**. No hay paso **Consultar el RUNT** y usted no escribe marca, línea, modelo ni organismo: FLITO los consulta después. Si el canal está apagado, en lugar del botón verá una tarjeta que lo explica; la cola sigue funcionando igual.
10. Como Cliente, si le rechazaron una solicitud, ábrala con **Ver**: verá la **causal** y la **observación** de Operaciones. Pulse **Corregir y reenviar** para arreglar lo señalado y enviarla de nuevo — es la **misma** solicitud, no una nueva, y vuelve a **Pendiente de revisión**. Tampoco aquí se pide **Consultar el RUNT**.
...
- El RUNT no bloquea el alta del canal **SOAT sin trámite**: un servicio caído, un vehículo sin registro o un SOAT vigente no impiden enviar. Eso lo ve Operaciones en la ficha del detalle, no el Cliente en el formulario.
