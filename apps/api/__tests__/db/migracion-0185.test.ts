// HU #12171 — La 0182: la tabla `permisos_auditoria` (ADR-0014), su inmutabilidad por REVOKE, la
...
const ARCHIVO = '0182_permisos_auditoria.sql';
...
describe('0182 — el archivo dice lo mismo que schema/permisos.ts (análisis estático)', () => {
  it('no trae control de transacción propio (ADR-DB-001) y los DO llevan dollar-quoting etiquetado, sin nombrar la etiqueta en comentarios', () => {
    ...
    expect(SQL_0182).toMatch(/DO \$revoke0182\$ BEGIN/);
    expect(SQL_0182).toMatch(/END \$revoke0182\$;/);
    expect(SQL_0182).toMatch(/DO \$resumen0182\$/);
    expect(SQL_0182).toMatch(/END \$resumen0182\$;/);
    expect(SIN_COMENTARIOS).not.toMatch(/\$\$/);
    const comentarios = SQL_0182.split('\n').filter((l) => l.trim().startsWith('--')).join('\n');
    expect(comentarios).not.toMatch(/\$revoke0182\$|\$resumen0182\$/);
  });
  ...
  it('el número es 0182 y no colisiona con ningún otro archivo', () => {
    const con0182 = readdirSync(path.dirname(RUTA)).filter((f) => f.startsWith('0182_'));
    expect(con0182).toEqual([ARCHIVO]);
  });
...
describe.skipIf(!URL_BASE)('0182 — contra la base real (constraints, ACL, retención, siembra e idempotencia)', () => {
  ...
  async function insertar(tx: postgres.TransactionSql, extra: Record<string, unknown>): Promise<{ code?: string } | null> {
    ...
    const fila = {
      lote_id: '00000000-0000-4000-8000-000000000182', entidad: 'usuario', accion: 'editar', campo: 'role',
      valor_antes: 'a', valor_despues: 'b', usuario_afectado_id: admin!.id, usuario_afectado_rol: 'admin',
      rol_afectado_codigo: null, actor_user_id: admin!.id, actor_email: 'prueba@0182', actor_rol: 'admin', origen: 'usuario',
      ...extra,
    };
