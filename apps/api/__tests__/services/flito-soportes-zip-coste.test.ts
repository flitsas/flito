// FLITO — coste REAL del ZIP de soportes (Feature #11908, HU #11910; topes Bug #12644).
//
...
// en el heap— y esto es otra cosa: I/O de MinIO y de FLIT, con `archiver` en store (zlib level 0)
// en el mismo hilo que atiende al resto del API y los bytes pasando en streaming sin quedarse.
// Los dos consumen el mismo proceso y ninguno predice al otro. Heredar los 2 000 «porque también
// es un archivo» habría sido estimar por analogía sobre otro mecanismo.
//
...
//     Con store (level 0) ya no hay `deflate` pesado; aun así el I/O síncrono del archivador
//     comparte el hilo, y la pregunta del AC3 de la HU #11651 vale igual: «ninguna petición en
//     vuelo del resto del sistema se pierde».
//   · **Bytes de salida y tiempo** — cuánto tarda el peor lote legal en llegar al cliente.
//     Si la duración se acerca a nginx `proxy_read_timeout` (~120 s), el escalón A síncrono
//     deja de ser viable y hay que pasar a ZIP asíncrono (fuera de este Bug).
//
// El escenario es el PEOR CASO LEGAL del Bug #12644: 300 registros × ~3 MiB ≈ 900 MB de contenido.
// El contenido es pseudoaleatorio a propósito: un PDF real ya viene comprimido, así que store no
// lo aplasta (igual que deflate) y el tamaño de salida ≈ entrada. Con texto repetido el número
// saldría optimista y el tope se fijaría sobre una medición que no describe el archivo real.
//
...
/** Peor lote legal del Bug #12644: 300 registros / ~900 MB. */
const ENTRADAS = 300;
/** ~3 MiB por documento → 300 × 3 MiB = 900 MiB de contenido. */
const BYTES_POR_DOCUMENTO = 3 * 1024 * 1024;

/**
 * Contenido pseudoaleatorio DETERMINISTA (xorshift32), no `crypto.randomBytes`.
 *
 * Determinista para que dos corridas midan lo mismo; **de verdad incompresible** para que el ZIP
 * en store no quede artificialmente pequeño: un PDF ya viene comprimido, y con contenido que se
 * comprimiría la medición saldría optimista por el lado de los bytes de salida.
 *
...
describe('coste del ZIP de soportes — peor lote legal Bug #12644 (~900 MB)', () => {
  // 180 s: generar + streamear ~900 MB puede acercarse al proxy_read_timeout de nginx (~120 s).
  // Si en CI/local la duración reportada se acerca a 120 s, documentarlo en el HANDOFF: el escalón
  // A síncrono queda al límite y el asíncrono es el siguiente paso (fuera de este Bug).
  it(`${ENTRADAS} entradas de ${mb(BYTES_POR_DOCUMENTO)} MB: RSS, lag del event loop y bytes`, async () => {
    ...
    // de ver si el archivador —trabajo en el mismo hilo que atiende al resto del API— deja de dar
    // paso a nadie mientras se construye el archivo.
    const PERIODO = 20;
    ...
    // Y el contenido es de verdad incompresible / store: el ZIP no debe pesar mucho menos que la
    // entrada (cabeceras ZIP añaden un poco; no restan).
    ...
    // `await stream.toArray()`, o encolar los 300 `append` sin esperar al evento `entry`— los
    // ~900 MB de documentos vivirían a la vez en el proceso y el delta pasaría de aquí. Con el tope
    // en 1 GiB ese mismo error costaría ~1 GB de los 262 de presupuesto: un `max_memory_restart`.
    //
    // Un umbral fijo del estilo «lo medido + 20 %» habría hecho otra cosa: dos corridas del mismo
    // código dan deltas muy distintos según dónde esté V8 en su ciclo (el mismo bimodalismo que
    // documenta `export-coste.ts`), así que ese umbral se «recalibraría» al primer rojo y dejaría
    // de decir nada.
    ...
    // se cuela. Esto lo atrapa sin ruido: como mucho UN documento abierto a la vez.
    ...
    // ser un test de cronómetro—, pero un archivador que monopolizara el hilo varios segundos
    // seguidos no llegaría ni a esto: con el archivador bloqueando, la fracción medida se desploma.
