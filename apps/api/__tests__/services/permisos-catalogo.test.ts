...
// CONJUNTOS contra la foto y contra los montajes leídos del fuente, no cardinales. El 47 sí se
// escribe: es 48 slugs de PAGES menos `flito_ayuda`, y ahí el número ES la afirmación del AC2-bis.
//
...
describe('AC2-bis — los dos defectos del catálogo de origen, y el que no lo era', () => {
  it('`PAGE_GROUPS` trae 48 entradas para 47 slugs únicos: `transito` está dos veces', () => {
    ...
    expect(entradas).toHaveLength(48);
    expect(new Set(entradas).size).toBe(47);
  ...
  it('las funciones de tipo `pagina` son 47: los 48 slugs de PAGES menos `flito_ayuda`', () => {
    expect(Object.keys(PAGES)).toHaveLength(48);
    expect(paginas).toHaveLength(47);
