...
//      `Content-Length` es el que corresponde a lo que el UTS municipal responde de verdad —los 5
//      municipios medidos contra el proveedor el 2026-08-21 traen `Content-Length` y ninguno
...
describe('httpsGetJson — un carácter multibyte partido entre dos trozos del socket', () => {
  ...
  it('tampoco con `Content-Length`, que es como responde el UTS de verdad', async () => {
    // El caso que desmonta el diagnóstico viejo del Bug #11711: los 5 municipios medidos el
    // 2026-08-21 responden con `Content-Length` y sin `Transfer-Encoding`, y aun así el cuerpo
