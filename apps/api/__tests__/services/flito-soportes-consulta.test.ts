...
describe('soportesDeSoat', () => {
  it('marca el origen para que el visor sepa agruparlo', async () => {
    ...
    const salida = await soportesDeSoat('s1', { rol: 'admin', externo: false, estadoSoat: 'pagado' });
  ...
  it('el comprobante del pago PSE solo entra para quien tiene derecho a verlo (HU #11678, AC5)', async () => {
    ...
    const conDerecho = await soportesDeSoat('s1', { rol: 'proveedor', externo: false, estadoSoat: 'pagado' });
    const auditoria = await soportesDeSoat('s1', { rol: 'auditor', externo: false, estadoSoat: 'pagado' });
