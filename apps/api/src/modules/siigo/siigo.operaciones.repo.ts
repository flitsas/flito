...
//
// El saneamiento del cuerpo es lo que hace la bitácora publicable: se registra lo suficiente para
// reconstruir qué pasó, sin dejar por escrito la clave con la que se factura ante la DIAN.
...
export async function registrarOperacion(r: RegistroOperacion): Promise<void> {
  try {
    ...
    console.error('[siigo] no se pudo registrar la operación en la bitácora', {
      operacion: r.operacion, resultado: r.resultado, err: e instanceof Error ? e.message : e,
    });
  }
}
