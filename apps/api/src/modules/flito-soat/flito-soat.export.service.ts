...
const COLUMNAS_CONSULTA = {
  ...
  // orígenes— y no por `flit_raw`. Los escribe el sync de FLIT (`numeroMotor`/`numeroSerie`, Bug
  // #12643) para los de trámite, y el RUNT donde ya escribía (HU #12401: canal Cliente al radicar,
  // certificación de Impuestos y recorrido de vigencia). Fuera de `DatosDeTramite` a propósito: no
  // dependen de la bifurcación por `origen`.
